...
def osint_agent(target: str) -> AgentResult:
    ...
    try:
        for rr in ('A', 'AAAA', 'MX', 'NS'):
            ...
            if answers: findings.append({'classification': 'FACT', 'type': 'dns', 'record': rr, 'answers': answers[:20]})
...
def vulnerability_agent(target: str, osint: AgentResult) -> AgentResult:
    """Agent 2: public CVE/CTI correlation. Keyword hits are never proof of exposure."""
    technologies: list[str] = []
    for finding in osint.findings:
        if finding.get('type') == 'web':
            for key in ('server', 'x_powered_by', 'generator'):
                value = finding.get(key)
                if value: technologies.append(str(value))
    ...
    for technology in technologies:
        ...
        try:
            ...
            for item in data.get('vulnerabilities', [])[:5]:
                ...
                if cid: findings.append({'classification': 'POSSIBLE', 'type': 'cve_candidate', 'technology': technology, 'cve': cid, 'description': description[:300], 'note': 'Public keyword correlation only; affected version/configuration must be verified.'})
...
def risk_agent(osint: AgentResult, vuln: AgentResult) -> AgentResult:
    ...
    return AgentResult(agent='RISK', status='completed', confidence=confidence, findings=[{'classification': 'FACT', 'type': 'risk_assessment', 'severity': severity, 'exposure_score': score, 'osint_coverage': osint_score, 'vulnerability_intel': vuln_score, 'reason': 'Evidence-weighted aggregation of Agents 1 and 2.'}], sources=list(dict.fromkeys(osint.sources + vuln.sources)))
...
def validation_agent(target: str, authorized_lab: bool) -> AgentResult:
    ...
    if not authorized_lab:
        return AgentResult(agent='VALIDATION', status='skipped', confidence='UNVERIFIED', findings=[{'classification': 'UNVERIFIED', 'type': 'validation_gate', 'reason': 'No explicit authorized lab scope supplied. No attack or exploitation performed.'}], sources=[])
